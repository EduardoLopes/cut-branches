...
- `@pindoba/devtools` (the live token editor, mounted dev-only in `src/components/app-shell.svelte`) is **not** part of the Panda build. It ships a precompiled stylesheet at `@pindoba/devtools/styles.css` (utilities + preview swatches, no tokens/reset), loaded via a `DEV`-guarded dynamic import next to the component. Never add a devtools buildinfo to `panda.config.ts` `include`. In local mode the stylesheet comes from `../pindoba/packages/devtools/dist/styles.css`, which only the devtools **build** (not `svelte-package --watch`) regenerates — run `pnpm --filter @pindoba/devtools build:css` in `../pindoba` after editing the panel's `css()` calls
