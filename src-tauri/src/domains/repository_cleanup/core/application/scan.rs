...
/// Find and size every cleanable folder in `repo_root`. Discovery is driven by
/// the repository's `.gitignore` stack (root, nested, and `.git/info/exclude`).
/// `on_progress(measured, current_path)` is called after each folder is measured
/// so the delivery layer can stream progress.
pub fn scan_cleanup_targets<F>(repo_root: &Path, mut on_progress: F) -> Vec<CleanupTarget>
where
    F: FnMut(u32, &Path),
{
    let found = scanner::find_cleanup_targets(repo_root);
