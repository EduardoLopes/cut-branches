...
import * as selectedBranchesStore from '$lib/stores/selected-branches.svelte';
...
vi.mock('$lib/components/branch-list.svelte', () => ({
...
vi.mock('$lib/components/repository-header.svelte', () => ({
	default: {
...
vi.mock('$lib/components/branches-bulk-actions.svelte', () => ({
	default: vi.fn().mockImplementation((options) => {
		const div = document.createElement('div');
