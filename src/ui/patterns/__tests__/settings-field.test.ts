...
describe('SettingsField', () => {
	it('renders the heading and subheading', () => {
		const screen = renderWithTestWrapper(SettingsField, {
			heading: 'Threshold',
	...
	it('renders a read-only value on the right when no control is given', () => {
		const screen = renderWithTestWrapper(SettingsField, {
			heading: 'App version',
			value: '0.5.0'
		});
	...
	it('renders a control snippet on the right, taking precedence over value', () => {
	...
	it('renders no trailing content when neither control nor value is provided', () => {
		const screen = renderWithTestWrapper(SettingsField, {
			heading: 'Info only',
