...
- **Infrastructure Layer** (how the feature talks to external systems):
  - `infrastructure/`: Code that reaches out to databases, external APIs, the filesystem, or IPC bindings. It translates external data into domain models. When the layer holds more than a couple of files, organize them into subfolders by the kind of external system they talk to — e.g., `infrastructure/repositories/`, `infrastructure/api/`, `infrastructure/git/`, `infrastructure/queries/`, `infrastructure/mutations/`. On the frontend, this includes server-state adapters — the query and mutation functions themselves (e.g., a `fetchBranches` function or a query-options factory). The hooks that _consume_ those adapters belong in `core/application/`, not here (see §1.2).
- **Shared / Utilities**:
...
- **Application Logic (`core/application/`):** Orchestrates the domain logic and connects Delivery to Infrastructure.
  ...
  - **Frontend State Management:** Distinguish between Server State and UI State, and split each cleanly.
    - **Server State (React Query / TanStack Query / Apollo / SWR / RTK Query) — hand-written.** The query/mutation function itself — the code that calls `fetch`, an IPC binding, or a generated client — is an _adapter_ and lives in `infrastructure/`. It is shaped exactly like a `SqlUserRepository`: it knows the wire format, knows the transport, translates errors, and does no orchestration. Co-locate the query-options factory (`{ queryKey, queryFn }`) with the adapter; the queryKey is part of the public contract of that adapter.
    - **The hook that consumes the adapter** (`useActiveBranchesView` calling `useQuery(listBranchesQuery())`, applying filters, deciding when to invalidate, exposing UI-shaped state) is the application layer. It lives in `core/application/` (or `core/composables/` if you prefer that name for hook-bound use-cases). This is where cache-invalidation policy lives, because invalidation rules are coordination, not transport.
    - Why split this way: the adapter is a plain async function — testable without React, reusable across `useQuery`/`useSuspenseQuery`/`prefetchQuery`/imperative `queryClient.fetchQuery`. Burying `useQuery` inside the same file as the fetch call closes those doors and forces every test to mount a `QueryClientProvider`. It also keeps hand-written `infrastructure/` files framework-agnostic — `useQuery` is React-specific and should not appear in code you wrote yourself.
    - **Server State — generated.** When TanStack hooks are _generated_ from a spec by a codegen tool (orval, openapi-codegen, kubb, hey-api, generated tRPC clients, etc.), do not artificially split the artifact. The generated bundle — `queryFn`, `queryKey`, and the `useQuery`/`useMutation` wrapper — is a single auto-regenerated unit, and **all of it lives in `infrastructure/`**. The framework-agnostic rule applies to code you _write_; generated code is owned by its generator and gets regenerated wholesale, so the React coupling is contained at a boundary you don't maintain by hand. The application-layer composable in `core/` then consumes the generated hook the same way it would consume a hand-written one.
    - **UI State** ("is this modal open?", "which tab is selected?") belongs strictly in the Delivery layer — local component state, or a UI-only store scoped to delivery code.
